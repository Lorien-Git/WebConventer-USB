...
{
	return (LastWinError ? LastWinError : default_error);
}
...
static errcode_t nt_open(const char *name, int flags, io_channel *channel)
{
    ...
    // Try to lock and dismount if we are opening for write access
    if (BooleanFlagOn(flags, EXT2_FLAG_RW)) {
        _LockDrive(nt_data->handle);
        _DismountDrive(nt_data->handle);
    }
...
errcode_t ext2fs_print_progress(int64_t cur_value, int64_t max_value) { return 0; }
